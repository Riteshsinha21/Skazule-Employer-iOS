...
class CustomAddShiftTempView: UIView {
    ...
    @IBOutlet weak var breakDurationBackView: UIView!
    ...
    func xibSetup() {
        ...
        self.checkInTextField.datePicker?.locale = Locale(identifier: "en_US_POSIX")
        self.checkInTextField.dateFormatter.dateFormat = "h:mm a"
        self.checkOutTextField.datePicker?.datePickerMode = .time
        self.checkOutTextField.datePicker?.locale = Locale(identifier: "en_US_POSIX")
    ...
    @objc func keyboardWillShow(notification: NSNotification) {
        if let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameBeginUserInfoKey] as? NSValue)?.cgRectValue {
            if self.view.frame.origin.y == 0{
                self.view.frame.origin.y -= keyboardSize.height - 160 //140
            }
